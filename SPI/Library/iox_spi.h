...
enum errors {
   ...
   ERR_DATA_NOT_RDY,       // IOX device is not ready to send data
};
...
#define MAX_BUFR_SIZE         64

#define SPI_TX_PACKET_SZ      12
#define SPI_RX_PACKET_SZ      12
...
#define PWM_CLK_48_MHZ		   0			// timer clock 0.0208333 usec
...
/** IOX_SPI class for communication to the IOX I/O co-processor **/
class IOX_SPI {
   public:
      IOX_SPI(void);            // constructor
      bool init(uint8_t spi_number, uint8_t pin_sck, uint8_t pin_miso, uint8_t pin_mosi, uint8_t pin_ss0, uint8_t pin_ss1, uint32_t bus_speed);
   ...
   private:
      uint8_t spi_send(uint8_t device_num, uint8_t *xmit_buf, uint8_t *rcv_buf, int8_t bytes2rcv);
      void _clear_tx_bufr(void);
      uint32_t _bus_speed;
      uint8_t _pin_sck;
      uint8_t _pin_miso;      
      uint8_t _pin_mosi;
      uint8_t _pin_ss0;                            // GPIO pin for device 0
      uint8_t _pin_ss1;                            // GPIO pin for device 1
      uint8_t _spi_num;                            // SPI ctlr FSPI or HSPI
      uint8_t _last_error;
